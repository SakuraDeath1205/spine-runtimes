...
void spine_shear_x_timeline_apply(spine_shear_x_timeline self, spine_skeleton skeleton, float lastTime, float time, /*@null*/ spine_array_event events, float alpha, spine_mix_from from, bool add, bool out, bool appliedPose) {
    ShearXTimeline *_self = (ShearXTimeline *) self;
...
float spine_shear_x_timeline_get_relative_value(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current, float setup) {
    ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getRelativeValue(time, alpha, (MixFrom)from, add, current, setup);
}

float spine_shear_x_timeline_get_absolute_value_1(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current, float setup) {
    ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getAbsoluteValue(time, alpha, (MixFrom)from, add, current, setup);
}

float spine_shear_x_timeline_get_absolute_value_2(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current, float setup, float value) {
    ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getAbsoluteValue(time, alpha, (MixFrom)from, add, current, setup, value);
}

float spine_shear_x_timeline_get_scale_value(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, bool out, float current, float setup) {
    ShearXTimeline *_self = (ShearXTimeline *) self;
